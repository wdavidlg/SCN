...
export const ModalTerapiaUpdate = () => {
  const { modalTerapiaUpdate } = useModalContext();
  const { values: form, handleInputChange, reset } = useForm(initialForm);
  const { state: {terapiaSelected}, update } = useTerapiaContext();
  const {
    state: { recordSelected },
  } = useRecordContext();

  useEffect(() => {
    if(!terapiaSelected) return
    reset(terapiaSelected)
  }, [terapiaSelected])

  const close = () => {
    reset();
    modalTerapiaUpdate.close();
  };
  ...
  return (
    <div>
      <Dialog open={modalTerapiaUpdate.visible}>
        <DialogTitle>Actualizar Terapia</DialogTitle>
        <DialogContent>
          <DialogContentText>
