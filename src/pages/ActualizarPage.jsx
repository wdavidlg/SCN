...
export const ActualizarPage = () => {
  const {state:{recordSelected} ,update } = useRecordContext();
  ...
  useEffect(() => {
    ...
    reset(recordSelected)
  }, [recordSelected])

  const handleClickSave = async() => {
    const {id, ...body} = form;
    const r = await update(id, body);
    if(r){
      reset();
      navigate("/ver")
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", marginBottom: 20 }}>
      {/* -----------------PACIENTE--------------------------- */}
        ...
        <Typography textAlign={"center"} fontWeight={"bold"}>ACTUALIZAR</Typography>
